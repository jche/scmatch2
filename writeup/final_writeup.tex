...
To illustrate an application of CSM, we analyze the canonical Lalonde dataset \citep{lalonde1986evaluating}.
...
Measured covariates include demographic information and past earnings.
The NSW dataset is experimental, but researchers typically add observational data to study whether methods can recover the experimental difference-in-means SATT estimate of \$1,794.
For this example, we estimate the SATT on the ``Dehejia and Wabha subsample'' of 185 NSW treated units \citep{dehejia1999causal}, using 15,992 observational control units from the Current Population Survey (CPS) dataset.\footnote{I.e., we exclude the experimental controls to make the analysis more challenging.}
For further details about the Lalonde dataset, we refer readers to the excellent overview provided in \citet{diamond2013genetic}.
...
Figure \ref{fig:lalonde_att} assesses the estimate-estimand tradeoff.
174 of the 185 treated units in the data have at least one control unit within the chosen caliper, so Figure \ref{fig:lalonde_att} visualizes the adaptive caliper sizes needed to estimate the SATT

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{writeup/figures/lalonde_att.png}
    \caption{Estimate-estimand tradeoff plot}
