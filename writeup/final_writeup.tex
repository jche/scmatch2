...
\subsection{Example: Lalonde (1986)}
\label{sec:lalonde}

\subsubsection{Background}

To illustrate the use of CSM, we analyze the canonical Lalonde datasets \citep{lalonde1986evaluating}.
...
Researchers typically augment the experimental National Supported Work Demonstration (NSWD) data with observational data to study whether methods can recover the experimental difference-in-means SATT estimate of \$1,794.
...
Unlike many analyses of these datasets, we exclude the 260 experimental NSWD control units.
As seen in Table \ref{tab:lalonde}, the treated units are generally much more similar to the NSWD controls than they are to the CPS controls;
in this example, however, we consider a more standard observational setting where we do not have access to such high-quality control units.
...
As discussed in Section \ref{sec:avoid} and shown in Proposition \ref{prop:wass}, distance-metric calipers directly guarantee approximate joint covariate balance, obviating the need to the usual marginal balance checks.
Nonetheless, Figure \ref{fig:lalonde_love} visualizes the ``love plot'' of marginal covariate means to highlight a few ideas.

\begin{figure}
    \centering
    \includegraphics{writeup/figures/lalonde_love2.png}
    \caption{Love plot of marginal covariate means}
    \label{fig:lalonde_love}
\end{figure}

Distance-metric calipers directly guarantee approximate joint covariate balance, so we do not need to check balance.
Regardless, we include love plots here for completeness.

LOVE PLOTS.

These love plots show that, marginally, balance is generally much better than our guarantees.
Importantly, marginal balance isn't the whole story.
While we visualize it because we're bad at visualizing higher dimensions, seeing good love plots here doesn't give us good joint balance!
Our calipers have already done that for us.

As we describe further in the following section, these love plots are, however, helpful for comparing the characteristics of the feasible and infeasible treated units, as well as their matched controls.
We emphasize that the role of these balance plots differs from their usual role.
By using distance-metric calipers, we ensure that the feasible joint distributions are close to each other.
We are using balance plots not to diagnose match quality, but to conduct exploratory data analysis into how the feasible units differ from the infeasible units.
We see that our feasible units tend to have better balance in terms of asdf and asdf.


To build intuition for the use of synthetic controls, we may assess how well our synthetic controls perform.
While these plots are not generally necessary, we find them helpful for building intuition here.
Figures \ref{fig:lalonde_dist} and \ref{fig:lalonde_ess} illustrate how synthetic controls impact the bias and variance of the FSATT estimate.
...
Generally speaking, convex weighted sums imply a linear interpolation to the covariates of the corresponding control ().\footnote{See Appendix \ref{app:scm} for details.}

Figure \ref{fig:lalonde_dist} shows density plots of the $d_V^{(\infty)}$ distances between the covariates of each treated unit $t$ and its corresponding control, constructed as $\sum_{j \in \Ct} w_{jt} \Xj$, $\frac{1}{|\Ct|} \sum_{j \in \Ct} \Xj$, and $\mathbf{X}_{j^*}$, for the ``SCM,'' ``Average,'' and ``1-NN'' estimation methods, respectively.
As expected, we see that these distances are much smaller for the synthetic controls, which are constructed precisely to minimize $d_V^{(\infty)}(\Xt, \sum_{j \in \Ct} w_{jt} \Xj)$.

\begin{figure}[t]
     \centering
     \begin{subfigure}[t]{0.48\textwidth}
         \centering
         \includegraphics[width=\textwidth]{writeup/figures/lalonde_dist.png}
     ...
     \begin{subfigure}[t]{0.48\textwidth}
         \centering
         \includegraphics[width=\textwidth]{writeup/figures/lalonde_ess.png}
         \caption{Effective sample size}
         \label{fig:lalonde_ess}
     \end{subfigure}
     % \caption{Three simple graphs}
     \label{fig:lalonde_bv}
\end{figure}


\subsubsection{Results}

Analyses of the Lalonde datasets typically show that causal-inference procedures recover estimates close to the experimental benchmark of \$1,794 while naive approaches (e.g., raw difference-in-means, linear regression) underestimate it.
Because individuals in the CPS dataset tend to have features associated with greater earnings, naive procedures often overestimate the counterfactual outcomes for the NSWD treated units, i.e., underestimate the treatment effect for those units.
...
If the FSATT and SATT intervals differ significantly, it is worth thinking about why this may be the case.
In our example, lower-quality matches may bias our effect estimates downward.
The differences between the NSWD and CPS datasets shown by Table \ref{tab:lalonde} lead us to expect that low-quality matches would pair NSWD individuals with low-earning characteristics to CPS individuals with higher-earning characteristics.
For example, we can check that the worst match (where $c_t = 2.62$) pairs an individual with zero earnings in 1974 and 1975 to someone with more than \$10,000 of earnings in both years.
This naturally leads to a very negative effect estimate for that individual, reducing the cumulative ATT estimate.

More generally, we would turn to exploratory plots such as figure LOVE PLOTS to continue with our diagnostics here.
Beyond checking imbalances, we can check if the feasible and infeasible treated units seem to significantly differ: if yes, heterogeneous effects might be at play, so the change in estimand might be a big deal.
If, on the other hand, it appears that it's mostly the feasible and infeasible control units that differ, we might be more worried about estimate bias.
