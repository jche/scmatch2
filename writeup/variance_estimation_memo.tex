...
For $k=1$ and $k=2$, a small minimum $k$ produces a tight caliper in dense regions, so $\tilde{N}_C$ is small when controls are concentrated.
...
The fair comparison therefore aligns scenarios by similar $\tilde{N}_C$ rather than by label.
