...
compute_coverage <- function(data, use_bias_correction = TRUE) {
  data %>%
    mutate(ATT = k * 1.5) %>%
    ...
    summarize(
      ...
      # Debiased vs non-debiased coverage
      coverage_rate = if(use_bias_correction) {
        mean((CI_lower - bias <= ATT) & (ATT <= CI_upper - bias), na.rm = TRUE)
      } else {
        mean((CI_lower <= ATT) & (ATT <= CI_upper), na.rm = TRUE)
      },
      avg_CI_width = mean((CI_upper - CI_lower), na.rm = TRUE),
      # Calculate empirical standard deviation
      emp_sd = sd(att_est - bias - ATT, na.rm = TRUE),
      # Theoretical standard deviation from SE
      theoretical_sd = mean(SE, na.rm = TRUE),
      # Average estimated V/n_T (from SE^2)
      mean_V_over_nT = mean(SE^2, na.rm = TRUE),
      # Empirical variance
      emp_var = var(att_est - bias - SATT, na.rm = TRUE) * 10,
...
# Compute coverage statistics with and without bias correction
...
summary_table <- coverage_with_bias_correction %>%
  select(
    deg_overlap,
    n_iterations,
    mean_SATT,
