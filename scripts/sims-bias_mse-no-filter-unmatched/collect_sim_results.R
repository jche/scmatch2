...
collect_sim_results <- function(sim_type, n_expected = 100) {
  ...
  results_dir <- file.path("data", "outputs", "sims-bias_mse-no-filter-unmatched", sim_type)
  ...
  output_file <- file.path("data", "outputs", "sims-bias_mse-no-filter-unmatched",
...
# Collect all simulationså
cat("\n", rep("=", 80), "\n", sep = "")
cat("COLLECTING ALL SIMULATION RESULTS\n")
cat(rep("=", 80), "\n", sep = "")

res_acic <- collect_sim_results("acic", n_expected = 1000)
res_hain <- collect_sim_results("hainmueller", n_expected = 1000)
