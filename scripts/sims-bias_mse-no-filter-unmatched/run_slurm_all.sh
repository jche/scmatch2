#!/bin/bash
#SBATCH -J all_sims-bias_mse-no-filter-unmatched
...
# Output written to data/outputs/sims-bias_mse-no-filter-unmatched/<sim>/iter_####.csv
# ---------------------------------------------------------------------
...
run_one() {
  ...
  echo "--- ${sim} / iter ${ITER} ---"
  if [[ -z "${METHODS}" ]]; then
    Rscript scripts/sims-bias_mse-no-filter-unmatched/run_single_iteration.R "${sim}" "${ITER}"
  else
    Rscript scripts/sims-bias_mse-no-filter-unmatched/run_single_iteration.R "${sim}" "${ITER}" "${METHODS_ARR[@]}"
  fi
}
