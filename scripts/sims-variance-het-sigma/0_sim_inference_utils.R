# scripts/sims-variance-het-sigma/0_sim_inference_utils.R
#
# Two heteroskedastic error scenarios:
#   Scenario 1 (sigma1_extra=0):   σ₀(x)=σ₁(x)=0.2+(x1-x2)²
#   Scenario 2 (sigma1_extra=0.5): σ₀(x)=0.2+(x1-x2)², σ₁(x)=σ₀(x)+0.5
#
# Runs four variance estimators:
#   homo        — hat_V_E_homo   (pooled S²)
#   het         — hat_V_E_het    (per-subclass variance)
#   alt_common  — hat_V_E_alt using S1²=mean(s_t²) [common-variance assumption]
#   alt_tt      — hat_V_E_alt using S1² from treated-to-treated K-NN
...
get_sim_paths <- function(output_name = "sims-variance-het-sigma") {
...
# -------------------------------------------------------------------
# Data generation: heteroskedastic errors
# -------------------------------------------------------------------

# σ₀(x) = 0.2 + (x1-x2)²
sigma0_fun <- function(x1, x2) { 0.2 + (x1 - x2)^2 }

make_het_sigma_df <- function(
    nc = 500, nt = 100,
    prop_nc_unif = 1/3,
    ctr_dist = 0.5,
    seed = NULL,
    sigma1_extra = 0   # 0 → scenario 1 (σ₁=σ₀); >0 → scenario 2
) {
  ...
  # Noiseless skeleton from gen_df_adv_k (f0_sd=0 gives zero noise)
  df_raw <- gen_df_adv_k(
    nc = nc, nt = nt, k = 2,
    f0_sd         = 0,
  ...
  sig0 <- sigma0_fun(df_raw$X1, df_raw$X2)
...
# -------------------------------------------------------------------
# Scaling (same spec as sims-variance)
# -------------------------------------------------------------------
...
toy_match_infer_het <- function(
  ...
  make_na_row <- function(method_name) {
    tibble(
      ...
      V_E = NA_real_, V_P = NA_real_,
      SATT = true_satt, bias = bias_val
    )
  }
  ...
  if (is.null(mtch)) {
    rs <- bind_rows(lapply(methods_all, make_na_row))
  } else {
    ...
    # V_P reference from pooled (treatment-effect heterogeneity, method-agnostic)
    ...
    row_alt_common <- tryCatch({
      ...
      V_E_c <- alt_res$V_E_alt
      SE_c  <- sqrt(pmax(0, V_E_c + V_P_ref))
      make_row("alt_common", SE_c, V_E_c, V_P_ref)
    }, error = function(e) {
      if (verbose) message("alt_common failed: ", e$message)
    ...
    row_alt_tt <- tryCatch({
      ...
      V_E_t <- alt_tt$V_E_alt
      SE_t  <- sqrt(pmax(0, V_E_t + V_P_ref))
      make_row("alt_tt", SE_t, V_E_t, V_P_ref)
    }, error = function(e) {
      if (verbose) message("alt_tt failed: ", e$message)
      make_na_row("alt_tt")
    })
  ...
  # Sample sizes (pulled once, shared across methods)
  N_T_val <- ESS_C_val <- NA_real_
  if (!is.null(mtch)) {
    sz <- tryCatch(
...
# -------------------------------------------------------------------
# sim_master_het: orchestration wrapper
# -------------------------------------------------------------------
...
sim_master_het <- function(
  ...
  result <- tryCatch(
    ...
    error = function(e) {
      warning("toy_match_infer_het failed iter ", iteration,
  ...
  result$time_secs <- as.numeric(difftime(Sys.time(), start_time, units = "secs"))
  result
}
