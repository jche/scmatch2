# scripts/sims-variance-het-sigma-bimodal/1_parallel_sim_inference.R
#
# SLURM usage:
#   sbatch run-adaptive-k2-het-sigma-bimodal.sh
#
# Direct usage:
...
output_name <- if (length(args) >= 2) args[[2]] else "sims-variance-het-sigma-bimodal"
...
for (overlap_label in overlap_labels) {
  for (sigma1_extra in sigma1_extras) {
    ...
    out_file  <- file.path(
      ...
      sprintf("bimodal_sigma_iter_%04d_overlap_%s_%s.rds", iter, overlap_label, sigma_tag)
    )
